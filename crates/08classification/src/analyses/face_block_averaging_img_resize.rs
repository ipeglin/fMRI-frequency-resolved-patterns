//! Analysis H — hammerAP task averaged image-resized: per-ROI rows from
//! `features/<src>/task_averaged_resized` over both CWT and HHT. Upstream
//! resizes each face-block raw spectrum to 224×224 then stack-means across
//! blocks before DenseNet, producing one image per ROI per subject.
...
pub fn run(cfg: &AppConfig) -> Result<()> {
    ...
    info!("starting task_averaged_resized classification");
    ...
    for source in [
        ...
        info!(
            ...
            "built task_averaged_resized dataset"
        );
        if xs.is_empty() {
    ...
    for source in [
        ...
        info!(
            ...
            samples = xs.len(),
            features = xs.first().map(|r| r.len()).unwrap_or(0),
            "built task_averaged_resized_mean dataset"
        );
        if xs.is_empty() {
            debug!(source = ?source, "no samples, skipping");
            continue;
