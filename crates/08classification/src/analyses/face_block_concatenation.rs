//! Analysis C — hammerAP task concat: per-ROI rows from
//! `features/<src>/task_concat` (deterministically shuffled face-block
//! concat) over both CWT and HHT.
...
pub fn run(cfg: &AppConfig) -> Result<()> {
    ...
    info!("starting task_concat (face) classification");
